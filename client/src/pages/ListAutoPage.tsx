...
import {ICar, IListAuto} from '../types/types';
import axios from "axios";
import DataGrid, {Column, Editing, Form, Lookup, Popup} from "devextreme-react/data-grid";
import 'devextreme/dist/css/dx.light.css';
import {Item} from "devextreme-react/form";

const ListAutoPage:FC = () => {

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const url: string = "http://localhost:3001/listauto";

    const urlCars: string = "http://localhost:3001/cars";
    ...
    const data = new CustomStore<IListAuto>({

        cacheRawData: false,

        errorHandler(error: Error) {console.log(error)},

        insert(values: IListAuto): PromiseLike<IListAuto> {
        ...
        load: function () {
                console.log(axios.get<IListAuto>(url));
                return axios.get<IListAuto>(url);
        },
        ...
        /*
        onInserted(values: IListAuto, key: any): void {
            console.log('Данные успешно добавлены')
        },

        onInserting(values: IListAuto): void {
        },

        onLoaded(result: Array<IListAuto>, loadOptions: LoadOptions<IListAuto>): void {
        },
        onLoading(loadOptions: LoadOptions<IListAuto>): void {
        },
        onModified(p0) {
        },
        onModifying(p0) {
        },
        onPush(changes: Array<IListAuto>): void {
        },
        onRemoved(key: any): void {
        },
        onRemoving(key: any): void {
        },
        onUpdated(key: any, values: IListAuto): void {
        },
        onUpdating(key: any, values: IListAuto): void {
        },
        */
        ...
        /*
        totalCount(loadOptions: { filter?: FilterDescriptor | Array<FilterDescriptor>; group?: GroupDescriptor<IListAuto> | Array<GroupDescriptor<IListAuto>> }): PromiseLike<number> {
            return Promise.resolve(0);
        },
        */

        /*
        update(key: any, values: IListAuto): PromiseLike<any> {
            return Promise.resolve(undefined);
        },
        */
    ...
    return (
        <div>
            <br/> <h1> Работа с гос. номерами</h1><br/>
            <DataGrid
            ...
            >
                ...
                <Column dataField="CarId" caption="Модель" width={450}>
                    <Lookup dataSource={store} valueExpr="id" displayExpr="name" />
                </Column>
                <Column dataField="numberPlate" caption='Государственый номер' width="290"/>
                <Editing
                    allowAdding={true}
                    allowDeleting={true}
                    mode="popup"
                >
                    <Popup title="Добавление государственного номера к авто" showTitle={true} width="750px" height="350px" />
                    <Form>
                        <Item itemType="group" colCount={1} colSpan={2}>
                            <Item dataField="CarId">
                                <Lookup dataSource={store} valueExpr="id" displayExpr="name" />
                            </Item>
                            <Item dataField="numberPlate"/>
                        </Item>
                    </Form>
                </Editing>
